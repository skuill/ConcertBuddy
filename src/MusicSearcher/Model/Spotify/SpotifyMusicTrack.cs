...
namespace MusicSearcher.Model.Spotify
{
    public class SpotifyMusicTrack : MusicTrackBase
    {
        ...
        public override string? TrackExternalLink =>
            _track.ExternalUrls.TryGetValue(EXTERNAL_URL_KEY, out string result)
            ? result
            : default;
        ...
        public override string? AlbumExternalLink =>
            _track.Album != null && _track.Album.ExternalUrls.TryGetValue(EXTERNAL_URL_KEY, out string result)
            ? result
            : default;

        public override IEnumerable<string>? ArtistsNames =>
            _track.Artists != null && _track.Artists.Count > 0
        ...
        public override IEnumerable<KeyValuePair<string, string>>? ArtistsExternalLinks =>
            _track.Artists != null && _track.Artists.Count > 0
            ? _track.Artists
                .Where(x => x.ExternalUrls.ContainsKey(EXTERNAL_URL_KEY))
